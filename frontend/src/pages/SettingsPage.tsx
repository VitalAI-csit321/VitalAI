...
function ManualImportSection() {
  const integrations = [
    { name: "CSV secure file intake", tags: ["ACTIVE", "MVP"], desc: "Connected and operational", active: true },
    { name: "Administrative upload queue", tags: ["ACTIVE", "MVP"], desc: "Connected and operational", active: true },
    { name: "EHR integration", tags: ["FUTURE STATE", "Concept"], desc: "Planned for future release", active: false },
    { name: "Laboratory systems", tags: ["FUTURE STATE", "Concept"], desc: "Planned for future release", active: false },
    { name: "SSO / Identity provider", tags: ["FUTURE STATE", "Concept"], desc: "Planned for future release", active: false },
  ];
  return (
    <div>
          ...
          <div key={int.name} className={`rounded-xl border p-4 ${int.active ? "border-slate-200 bg-white" : "border-dashed border-slate-200 bg-white"}`}>
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="flex gap-2 mb-1 ml-4">
                  {int.tags.map(t => (
                    <span key={t} className={`rounded px-1.5 py-0.5 text-xs font-semibold ${t === "ACTIVE" ? "bg-emerald-100 text-emerald-700" : t === "MVP" ? "bg-slate-100 text-slate-600" : "bg-slate-100 text-slate-500"}`}>{t}</span>
                  ))}
                </div>
                <p className="text-sm text-slate-500 ml-4">{int.desc}</p>
              </div>
              {int.active && <button className="rounded-lg bg-brand px-4 py-1.5 text-sm font-semibold text-white hover:bg-brand-hover ml-4">Configure</button>}
            </div>
          </div>
        ))}
      </div>
      <div className="mt-4 rounded-lg border border-brand/20 bg-emerald-50 p-4 flex items-start gap-3">
        <span className="text-brand text-lg">ⓘ</span>
        <div>
          <p className="text-sm font-semibold text-brand">MVP phase active</p>
          <p className="text-sm text-emerald-700">Manual data import is the current integration method. Future state integrations will be enabled in subsequent releases.</p>
        </div>
      </div>
    </div>
  );
