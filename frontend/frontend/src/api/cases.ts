...
export async function createPatientFromOnboarding(input:{
  ...
  // Also create an intake case linked to this patient
  try {
    await apiPost("/api/v1/intake",{
      ...
      notes:JSON.stringify({address:input.address,indigenous_status:input.indigenousStatus,preferred_language:input.preferredLanguage}),
    });
  } catch { /* best-effort */ }
